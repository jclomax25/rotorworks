...
def motor_shaft_power_from_thrust(config: FixedWingConfig, thrust_N: float) -> float:
    ...
    if prop.table is not None:
        # --- Test-table mode: interpolate Power_W from Thrust_g ---
        thrust_g = thrust_N * 1000.0 / G0
        df       = prop.table
        if thrust_g <= df["Thrust_g"].iloc[0]:
            return float(df["Power_W"].iloc[0])
        if thrust_g >= df["Thrust_g"].iloc[-1]:
...
def launch_gui():
    ...
    tab_env      = make_scrollable_tab(input_nb, "Environment")
    ...
    config_vars = dict(
        ...
        wind=v_wind, max_v_plot=v_max_v_plot, periph_cur=v_periph_cur,
    )
    ...
    for t in (tab_plots, tab_status, tab_metrics):
    ...
    def _sep_metric(label=""):
        metrics_tv.insert("", "end", values=(f"── {label} ──", ""))
    ...
    def build_config() -> FixedWingConfig:
        ...
        batt = BatteryConfig(
            ...
            unit_mode               = v_batt_unit_mode.get() or "cell",
    ...
    def run():
        ...
        try:
            ...
            # Wind correction for range/endurance (headwind increases effective fuel burn per km)
            ...
            # Save state so View menu changes can re-render immediately
            _last_run["cfg"]     = cfg
            ...
            summary = (
                f"=== Fixed-Wing Analysis @ {V_cruise:.1f} m/s ({V_cruise*3.6:.1f} km/h) ===\n"
                f"Stall Speed   : {V_stall:.1f} m/s\n"
                f"L/D Ratio     : {m['LD_ratio']:.2f}\n"
                f"Thrust Req    : {m['thrust_required_N']:.2f} N  |  Avail: {m['thrust_available_N']:.2f} N\n"
                f"Total Power   : {m['total_power_W']:.1f} W\n"
                f"Flight Time   : {m['flight_time_min']:.1f} min\n"
                f"Flight Range  : {m['flight_range_km']:.2f} km\n"
                f"Rate of Climb : {m['rate_of_climb_mps']*60:.0f} m/min\n"
                f"Max RC        : {m['max_rc_mps']*60:.0f} m/min  @ {m['v_max_rc_mps']:.1f} m/s\n"
                f"Best Endurance: {V_be:.1f} m/s  |  Best Range: {V_br:.1f} m/s\n"
                f"Takeoff Roll  : {m['takeoff_dist_m']:.1f} m\n"
            )
            out_print(summary)
        except Exception as e:
            import traceback
    ...
    ttk.Button(btn_frame, text="▶  Run Simulation", command=run).grid(
    ...
    ttk.Button(btn_frame, text="💾  Save Config", command=save_cfg).grid(row=0, column=2, padx=4)
    ttk.Button(btn_frame, text="📂  Load Config", command=load_cfg).grid(row=0, column=3, padx=4)
